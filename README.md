...
> 💡 When setting the `expires` meta field, the string _"(update frequency)"_ is optional in `ADBT` files. In Adblock filter files is requires, hence, it will be automatically added to the compiled filter file by the compiler.
